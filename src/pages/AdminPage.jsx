    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-30">
      <div className="bg-white w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-lg border border-gray-200">
        ...
        <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {images.map((img, idx) => {
            const fullImageUrl = `${SERVER_BASE_URL}/uploads/${img.filename}`;
            return (
              <div
                key={idx}
              ...
              >
                ...
                <p className="text-xs text-gray-700 text-center mb-1">
                  Size: {img.size} | Qty: {img.quantity}
                </p>
                <p className="text-xs text-gray-700 text-center mb-1">
                  Paper: {img.paperType || "N/A"}
                </p>
                <button
...
const OrderDetailsModal = ({ order, onClose, onViewImages }) => {
  ...
  const paperTypes = order.items?.map((i) => i.paperType) || [];
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-30">
      <div className="bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-lg border border-gray-200 p-6">
        ...
        <div className="space-y-2 text-gray-800">
          ...
          <p><strong>Address:</strong> {order.deliveryAddress?.street}, {order.deliveryAddress?.city}, {order.deliveryAddress?.emirate}</p>
          <p>
            <strong>Paper Types:</strong>{" "}
            {paperTypes.length > 0 ? paperTypes.join(", ") : "N/A"}
          </p>
        ...
        {/* View Images Button */}
        <button
          onClick={() => onViewImages(order.items)}
        ...
        >
          <FaImages /> View Images ({order.items?.length || 0})
        </button>
      </div>
    </div>
  );
...
function AdminPage() {
            ...
            >
              ...
              <div>
                ...
                <p className="text-gray-700 mb-1">
                  <span className="font-medium text-gray-900">Customer:</span> {order.deliveryAddress?.name}
                </p>
                <p className="text-gray-700 mb-1">
                  <span className="font-medium text-gray-900">Phone:</span> {order.deliveryAddress?.phone}
                </p>
                <p className="text-gray-700 mb-1">
                  <span className="font-medium text-gray-900">Address:</span> {order.deliveryAddress?.street}, {order.deliveryAddress?.city}, {order.deliveryAddress?.emirate}
                </p>
                <p className="text-gray-700 mb-1">
                  <span className="font-medium text-gray-900">Paper Types:</span>{" "}
                  {paperTypes.length ? paperTypes.join(", ") : "N/A"}
                </p>
                <p className="text-gray-700 mb-1">
              ...
              >
                <FaImages /> View Files ({order.items?.length || 0})
              </button>
            </div>
          );
        })}
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
        ...
        <ul className="mt-4">
          {tabs.map((tab) => (
            <li
              ...
              className={`flex items-center gap-3 p-4 cursor-pointer hover:bg-gray-100 transition ${activeTab === tab.id ? "bg-gray-200 font-semibold" : ""}`}
            >
              {tab.icon} {tab.label}
            </li>
          ))}
      ...
      {selectedOrder && (
        <OrderDetailsModal order={selectedOrder} onClose={closeOrderDetails} onViewImages={openImageModal} />
      )}
    </div>
  );
}
